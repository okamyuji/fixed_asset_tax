...
- **減価償却計算**: 国税庁公式テーブルに基づく5方式対応の計算エンジン
...
取得時期に応じた正確な償却方式を実装しています。全償却率は国税庁公式テーブル（耐用年数2〜50年）から取得し、共有モジュール `Tax::DepreciationRates` で一元管理しています。

- **旧定額法**（H19.3.31以前取得）: 基礎額(取得価額×90%)×償却率、95%到達後は均等償却5年→備忘価額1円
- **旧定率法**（H19.3.31以前取得）: 未償却残高×償却率、95%到達後は均等償却5年→備忘価額1円
- **定額法**（H19.4.1以降取得）: 取得価額×償却率、備忘価額1円まで償却
- **250%定率法**（H19.4.1〜H24.3.31取得）: 未償却残高×償却率、保証額未満で改定償却率に切替
- **200%定率法**（H24.4.1以降取得）: 未償却残高×償却率、保証額未満で改定償却率に切替

- 取得時期による自動方式判定（`DepreciationRates.recommended_method`）
- 250%/200%定率法: 償却保証額・改定償却率テーブル完備
- 特殊償却タイプ: 即時償却、一括償却（3年均等）、少額減価償却、特別償却、割増償却

#### 2. 固定資産税評価上の減価計算

会計上の減価償却とは別に、地方税法に基づく固定資産税評価額を算出します。

- 旧定率法の償却率テーブル（耐用年数2〜50年の完全網羅）を使用
- 初年度の半年償却（評価額 = 取得価額 × (1 - 減価率 × 0.5)）
- 2年目以降の計算（評価額 = 前年度評価額 × (1 - 減価率)）
- 最低限度額（取得価額の5%）の適用
- 賦課期日（1月1日）ルール: 1月1日取得は同年度初年度、1月2日以降取得は翌年度初年度
- 東京都主税局手引きの計算例3件で検証済み
...
- バックエンドユニットテスト: 108件、1081アサーション
    - 減価償却計算: 27件（5方式 + 特殊償却タイプ + エッジケース）
    - 固定資産税評価: 20件（東京都手引き計算例3件含む）
    - 課税標準額: 22件
    - 償却率テーブル: 検証テスト完備
...
- **Tax::DepreciationRates**: 国税庁公式償却率テーブル（5方式、耐用年数2〜50年）
- **Tax::DepreciationCalculator**: 会計上の減価償却計算（5方式対応）
- **Tax::FixedAssetTaxValuationCalculator**: 固定資産税評価上の減価計算
- **Tax::PropertyTaxCalculator**: 固定資産税額計算（特例・免税点対応）
- **Tax::CorporateScheduleGenerator**: 法人税別表十六の自動生成
...
**テスト結果**: 108 runs, 1081 assertions, 0 failures, 0 errors, 0 skips
