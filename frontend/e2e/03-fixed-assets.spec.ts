...
test.describe('固定資産CRUD', () => {
    ...
    test('固定資産一覧ページに遷移できる', async ({ page }) => {
        ...
        await expect(page.getByRole('heading', { name: '固定資産管理' })).toBeVisible();
    ...
    test('空の固定資産一覧が表示される', async ({ page }) => {
        ...
        // 空メッセージを確認
        await expect(page.getByText('登録されている固定資産がありません')).toBeVisible();
    });
    ...
    test('新規固定資産作成ページに遷移できる', async ({ page }) => {
        ...
        await expect(page.getByRole('heading', { name: '固定資産の新規登録' })).toBeVisible();
    });
    ...
    test('固定資産編集ページに遷移できる', async ({ page }) => {
        ...
        await expect(page.getByRole('heading', { name: '固定資産の編集' })).toBeVisible();
    });
