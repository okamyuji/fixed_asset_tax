...
async function setupCalculationResultPage(page: Page, calculationId: number, results: CalculationResult[]) {
    // 先にAPIモックを設定
    await page.route(`**/api/v1/calculation_runs/${calculationId}?*`, async (route) => {
        await route.fulfill({
            status: 200,
            contentType: 'application/json',
            body: JSON.stringify({
    ...
    await page.route(`**/api/v1/calculation_runs/${calculationId}/results*`, async (route) => {
        await route.fulfill({
            status: 200,
            contentType: 'application/json',
    ...
    await page.goto(`/frontend/calculations/${calculationId}/results`, { waitUntil: 'networkidle' });
}
...
test.describe('日本の税制準拠検証（MECE）', () => {
    ...
    test('シナリオ2.1: 小規模住宅用地（200㎡以下）の1/6特例', async ({ page }) => {
    ...
    test('シナリオ5.1: 初年度の定率法計算（半年償却）', async ({ page }) => {
        ...
        await expect(page.getByText('テスト償却資産（初年度定率法）')).toBeVisible();
        await expect(page.getByText('¥125,580').first()).toBeVisible();
    });
