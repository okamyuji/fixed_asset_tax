...
export const assetClassificationsApi = {
  ...
  getAccountItems: async () => {
    const response = await apiClient.get('/asset_classifications/account_items');
    return response.data;
  },

  getUsefulLife: async (accountItem: string): Promise<{ account_item: string; useful_life_years: number | null }> => {
