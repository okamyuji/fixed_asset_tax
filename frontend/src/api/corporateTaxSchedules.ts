import type {
	CorporateTaxSchedule,
	CorporateTaxScheduleCreateRequest,
} from "../types";
...
export const corporateTaxSchedulesApi = {
	...
	create: async (
		data: CorporateTaxScheduleCreateRequest,
	): Promise<CorporateTaxSchedule> => {
	...
	update: async (
		id: number,
		data: Partial<CorporateTaxScheduleCreateRequest>,
	): Promise<CorporateTaxSchedule> => {
	...
	generate: async (
		...
		const response = await apiClient.post(
			`/corporate_tax_schedules/${id}/generate`,
			{
	...
	generateAll: async (
		...
		const response = await apiClient.post(
			"/corporate_tax_schedules/generate_all",
			{
				fiscal_year_id: fiscalYearId,
			},
		);
	...
	finalize: async (id: number): Promise<CorporateTaxSchedule> => {
		const response = await apiClient.post(
			`/corporate_tax_schedules/${id}/finalize`,
		);
		return response.data;
	},
	...
	exportCsv: async (id: number): Promise<Blob> => {
		const response = await apiClient.get(
			`/corporate_tax_schedules/${id}/export_csv`,
			{
				responseType: "blob",
			},
		);
