...
export const useCalculations = () => {
    ...
    const { data: calculationRuns = [], isLoading, error } = useQuery({
    ...
    const createMutation = useMutation({
        mutationFn: (data: CalculationRunCreateRequest) => calculationsApi.create(data, tenantId!),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['calculationRuns', tenantId] });
        },
    });
