...
import { useCalculationResults, useCalculationRun } from '../hooks/useCalculations';

export const CalculationResultsPage = () => {
    const { id } = useParams<{ id: string }>();
    const { data: calculationRun, isLoading: isLoadingRun } = useCalculationRun(Number(id));
    const { data: results = [], isLoading: isLoadingResults } = useCalculationResults(Number(id));
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">
                        計算ID: {calculationRun.id} | 市区町村ID: {calculationRun.municipality_id} | 年度ID: {calculationRun.fiscal_year_id}
                    </p>
                </div>
                <Link
                    to="/calculations"
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                        <thead className="bg-slate-50">
                            <tr>
                                <th scope="col" className="py-4 pl-6 pr-3 text-left text-sm font-semibold text-slate-900">
                                    資産名
                                </th>
                                ...
                                <th scope="col" className="px-3 py-4 pr-6 text-left text-sm font-semibold text-slate-900">
                                    税額
                                </th>
                            </tr>
                        </thead>
